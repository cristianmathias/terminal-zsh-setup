...
BREWFILE="${SCRIPT_DIR}/Brewfile.ubuntu"
ZSHRC="${HOME}/.zshrc"
...
usage() {
  cat <<'EOF'
Usage:
  ./terminal-zsh-setup/install-terminal-zsh-setup-ubuntu.sh [options]

Options:
  --no-packages      Do not install apt/Homebrew packages. Only apply shell/Git config.
...
What this script does on Ubuntu:
  - installs apt prerequisites for Homebrew/Linuxbrew;
  - installs Homebrew/Linuxbrew if missing;
  - installs packages from terminal-zsh-setup/Brewfile.ubuntu;
  ...
  - creates ~/.gitignore_global with common ignores;
...
require_ubuntu() {
  if [[ "$(uname -s)" != "Linux" ]]; then
    printf 'This setup is intended for Ubuntu/Linux.\n' >&2
    exit 1
  fi
  ...
  if [[ -r /etc/os-release ]]; then
    ...
    if [[ "${ID:-}" != "ubuntu" && "${ID_LIKE:-}" != *"ubuntu"* && "${ID_LIKE:-}" != *"debian"* ]]; then
      warn "This script was designed for Ubuntu. Detected: ${PRETTY_NAME:-unknown Linux}"
    fi
  fi
}

install_apt_prerequisites() {
  [[ "$INSTALL_PACKAGES" -eq 1 ]] || return
...
ensure_homebrew() {
  ...
  if [[ -x /home/linuxbrew/.linuxbrew/bin/brew ]]; then
    ...
    printf 'Homebrew was not found at /home/linuxbrew/.linuxbrew/bin/brew after install.\n' >&2
    exit 1
  fi
}
...
install_nerd_font() {
  ...
  local tmpdir
  tmpdir="$(mktemp -d)"
  local fontdir="${HOME}/.local/share/fonts/JetBrainsMonoNerdFont"

  mkdir -p "$fontdir"
  curl -fL "$url" -o "$tmpdir/JetBrainsMono.zip"
  unzip -o "$tmpdir/JetBrainsMono.zip" -d "$fontdir" >/dev/null
  fc-cache -f "$fontdir" >/dev/null 2>&1 || fc-cache -f >/dev/null 2>&1 || true
  rm -rf "$tmpdir"
}
...
if [[ -r /home/linuxbrew/.linuxbrew/share/zsh-autosuggestions/zsh-autosuggestions.zsh ]]; then
...
if [[ -r /home/linuxbrew/.linuxbrew/share/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh ]]; then
...
verify_setup() {
  log "Verifying setup"
  zsh -n "$ZSHRC"
  printf '\nDone. Set your terminal font to JetBrainsMono Nerd Font Mono, open a new tab, and run: ll\n'
}
